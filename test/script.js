document.addEventListener('DOMContentLoaded', function() {
            ...
            async function loadData() {
                ...
                const issuePromises = sourceNames.map(sourceName => {
                    ...
                    return fetch(url)
                        ...
                        .catch(error => console.error('Error loading data from ' + url, error));
                });
            ...
            function openCombinedView(bugId) {
                ...
                fetch(`https://api.test.osv.dev/v1/vulns/${bugId}`)
                    .then(res => res.ok ? res.json() : { error: `Failed to load: ${res.status}` })
                    .then(data => {
                        document.getElementById(`vuln-json-${bugId}`).textContent = JSON.stringify(data, null, 2);
                    })
                    .catch(err => {
                        document.getElementById(`vuln-json-${bugId}`).textContent = `Error: ${err.message}`;
                    });
                ...
                if (details) {
                    ...
                    findingsPre.textContent = 'No linter findings available for this vulnerability.';
                }
            }
            ...
            function setActiveTab(tabId) {
                // Buttons
                Array.from(tabBarContainer.querySelectorAll('.tab-switch-button')).forEach(btn => {
