...
var DebianCopyrightDenylist = []conversion.VendorProduct{
...
func outputProductToRepoMap(prm conversion.VendorProductToRepoMap, f io.Writer) error {
...
func analyzeCPEDictionary(cpes []CPE) (productToRepo conversion.VendorProductToRepoMap, descriptionFrequency map[string]int) {
	productToRepo = make(conversion.VendorProductToRepoMap)
	descriptionFrequency = make(map[string]int)
	MaybeTryDebian := make(map[conversion.VendorProduct]bool)
	for _, c := range cpes {
		if c.Deprecated {
			logger.Info("Skipping deprecated", slog.String("cpe", c.Name))
			continue
		}
		parsedCPE, err := conversion.ParseCPE(c.Name)
		if err != nil {
		...
		for _, r := range c.References {
			...
			repo, err := conversion.Repo(r.URL)
			if err != nil {
			...
			if slices.Contains(productToRepo[conversion.VendorProduct{Vendor: parsedCPE.Vendor, Product: parsedCPE.Product}], repo) {
				continue
			}
			...
			productToRepo[conversion.VendorProduct{Vendor: parsedCPE.Vendor, Product: parsedCPE.Product}] = append(productToRepo[conversion.VendorProduct{Vendor: parsedCPE.Vendor, Product: parsedCPE.Product}], repo)
			// If this was queued for trying to find via Debian, and subsequently found, dequeue it.
			if *DebianMetadataPath != "" {
				delete(MaybeTryDebian, conversion.VendorProduct{Vendor: parsedCPE.Vendor, Product: parsedCPE.Product})
			}
		}
		...
		if len(productToRepo[conversion.VendorProduct{Vendor: parsedCPE.Vendor, Product: parsedCPE.Product}]) == 0 && *DebianMetadataPath != "" {
			// Check the denylist though.
			if slices.Contains(DebianCopyrightDenylist, conversion.VendorProduct{Vendor: parsedCPE.Vendor, Product: parsedCPE.Product}) {
				continue
			}
			MaybeTryDebian[conversion.VendorProduct{Vendor: parsedCPE.Vendor, Product: parsedCPE.Product}] = true
		}
	}
	...
	if len(MaybeTryDebian) > 0 && *DebianMetadataPath != "" {
		...
		for vp := range MaybeTryDebian {
			...
			if repo != "" {
				...
				productToRepo[conversion.VendorProduct{Vendor: vp.Vendor, Product: vp.Product}] = append(productToRepo[conversion.VendorProduct{Vendor: vp.Vendor, Product: vp.Product}], repo)
			}
		}
	}
...
func validateRepos(prm conversion.VendorProductToRepoMap) (validated conversion.VendorProductToRepoMap) {
	validated = make(conversion.VendorProductToRepoMap)
