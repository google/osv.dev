...
def main():
    ...
    prs_output = run_gh_command(["pr", "list", "--label", "dependencies", "--state", "open", "--json", "number,title,headRefName,url"])
    ...
    for pr in prs:
        ...
        status_output = run_gh_command(["pr", "view", str(pr_num), "--json", "statusCheckRollup"])
        ...
        for check in checks:
            ...
            if status != "COMPLETED":
                ...
                failed_or_pending_checks.append(f"{check.get('name')} (Failed: {conclusion})")
        ...
        if failed_or_pending_checks:
            reasons_for_manual.append(f"Failing/Pending CI ({len(failed_or_pending_checks)} checks)")
        if is_major:
    ...
    print("These PRs are patch or minor updates with passing CI and standard file changes.")
    ...
    print("These PRs require manual intervention due to major version jumps, unusual modifications, or failing CI checks.")
