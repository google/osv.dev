...
def safe_http_call(call_func, error_prefix: str = "Error") -> dict[str, Any]:
    ...
    try:
        ...
        error_msg = f"{error_prefix}: HTTP {e.response.status_code}: {e.response.text}"
...
def get_vulnerability_by_id(vuln_id: str) -> dict[str, Any]:
    ...
    Args:
        vuln_id: The OSV vulnerability ID (e.g., "GHSA-xxxx-xxxx-xxxx" or "CVE-2024-1234")
...
def query_affected(
    ...
    return safe_http_call(
        lambda: session.post(url, json=query),
...
def determine_version(
    ...
    return safe_http_call(
        lambda: session.post(url, json=version_query),
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="127.0.0.1",
        help="Host to run MCP server on (only for SSE transport), default: 127.0.0.1"
    )
    parser.add_argument(
        "--mcp-port",
        type=int,
        default=8001,
        help="Port to run MCP server on (only for SSE transport), default: 8001"
    )
    parser.add_argument(
        "--transport",
        type=str,
    ...
    if args.transport == "sse":
        try:
            ...
            logger.info(f"Connecting to OSV API at {api_endpoint}")
