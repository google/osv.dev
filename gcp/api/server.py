...
def build_determine_version_result(
  ...
  for f in idx_futures:
    ...
    # gives a 0, but the bitmap of the repo is a 1. We do not want to count
    # cases where bitmap of the repo is a 0, and the user query returns is 1,
    # since those cases would have already been accounted for when querying
    # the user queries bucket and missing. A bitwise NOT on the user query
    # bitmap followed by a bitwise AND satisfies this requirement.
...
def estimate_diff(num_of_bucket_change: int, file_count_diff: int) -> int:
  """
