...
def run():
  ...
  process_oss_fuzz(oss_fuzz_source)


def process_oss_fuzz(oss_fuzz_source):
  ...
  #
  # This then becomes the source of truth where any edits are imported back into
  # OSV.
  with tempfile.TemporaryDirectory() as tmp_dir:
    repo = osv.clone_with_retries(oss_fuzz_source.repo_url, tmp_dir)
    if not repo:
    ...
    repo.create_commit('refs/heads/main', author, author,
                       'Import from OSS-Fuzz', tree, [repo.head.peel().oid])
