...
func CVEToOSV(cve models.NVDCVE, repos []string, cache *git.RepoTagsCache, directory string, metrics *models.ConversionMetrics, rejectFailed bool, outputMetrics bool) models.ConversionOutcome {
	...
	r, un, sR := conversion.ProcessRanges(cpeRanges, repos, metrics, cache, models.VersionSourceCPE)
	...
	if len(resolvedRanges) == 0 {
		...
		r, un, sR := conversion.ProcessRanges(textRanges, repos, metrics, cache, models.VersionSourceDescription)
		if metrics.Outcome == models.Error {
			return models.Error
		}
		resolvedRanges = append(resolvedRanges, r...)
	...
	affected := conversion.MergeRangesAndCreateAffected(groupedRanges, commits, keys, metrics)
