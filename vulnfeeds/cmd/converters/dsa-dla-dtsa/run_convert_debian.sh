...
CONVERT_CMD="go run main.go"
if [ -f "./convert_debian_bin" ]; then
  CONVERT_CMD="./convert_debian_bin"
fi

echo "Converting DSAs"
$CONVERT_CMD --adv_type=DSA -o $OSV_DSA_OUT $WEBWML_PATH $DSA_PATH
echo "Converting DLAs"
$CONVERT_CMD --adv_type=DLA -o $OSV_DLA_OUT $WEBWML_PATH $DSA_PATH
echo "Converting DTSAs"
$CONVERT_CMD --adv_type=DTSA -o $OSV_DTSA_OUT $WEBWML_PATH $DSA_PATH
