...
def analyze_pr(pr: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if unexpected_files:
        reasons_for_manual.append(f"Unexpected files: {', '.join(unexpected_files)}")
...
def generate_report(ready_prs: List[Dict], manual_prs: List[Dict]):
    ...
    print(
        "These PRs are patch or minor updates with passing CI and "
    ...
    print(
        "These PRs require manual intervention due to major version jumps, "
...
def main():
    ...
    parser.add_argument(
        "--approve", action="store_true", help="Approve ready PRs (use with caution)"
    )
    parser.add_argument(
        "--merge",
        action="store_true",
