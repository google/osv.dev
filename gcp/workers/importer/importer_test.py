...
@mock.patch('importer.utcnow', lambda: datetime.datetime(2021, 1, 1, tzinfo=datetime.UTC))
class ImporterTest(unittest.TestCase, tests.ExpectationTest(TEST_DATA_DIR)):
  ...
  def test_basic(self, unused_mock_time: mock.MagicMock,
    ...
    osv.Bug(
        ...
        timestamp=datetime.datetime(2021, 1, 15, 0, 0, 24, 559102, tzinfo=datetime.UTC),
        database_specific={
            'database_specific': 1337
        },
    ).put()
  ...
  def test_scheduled_updates(self, unused_mock_time: mock.MagicMock,
    ...
    osv.Bug(
        ...
        timestamp=datetime.datetime(2020, 1, 1, 0, 0, 0, 0, tzinfo=datetime.UTC)).put()
    osv.Bug(
        db_id='OSV-2021-1338',
        affected_packages=[
            osv.AffectedPackage(
    ...
    source_repo = osv.SourceRepository.get_by_id('oss-fuzz')
    self.assertEqual(
        datetime.datetime(2021, 1, 1, 10, 0, tzinfo=datetime.UTC), source_repo.last_update_date)
...
class BucketImporterTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    osv.Bug(
        ...
        # file
        import_last_modified=datetime.datetime(2014, 9, 20, 8, 18, 7, 0, tzinfo=datetime.UTC),
    ).put()
    ...
    osv.Bug(
        ...
        import_last_modified=datetime.datetime(2018, 2, 9, 3, 29, 0, 0, tzinfo=datetime.UTC),
    ).put()
  ...
  def test_blob_parsing(self):
    ...
    if not self.source_repo.last_update_date:
      self.source_repo.last_update_date = datetime.datetime.min.replace(tzinfo=datetime.UTC)
...
@mock.patch('importer.utcnow', lambda: datetime.datetime(2024, 1, 1, tzinfo=datetime.UTC))
class RESTImporterTest(unittest.TestCase):
  ...
  def test_all_updated(self, unused_mock_time: mock.MagicMock,
    ...
    self.source_repo.last_update_date = datetime.datetime(2020, 1, 1, tzinfo=datetime.UTC)
  ...
  def test_last_update_ignored(self, unused_mock_time: mock.MagicMock,
    ...
    data_handler.last_modified = 'Mon, 01 Jan 2024 00:00:00 GMT'
    data_handler.load_file(data_handler, 'rest_test.json')
    self.source_repo.last_update_date = datetime.datetime(2023, 6, 6, tzinfo=datetime.UTC)
    self.source_repo.ignore_last_import_time = True
    repo = self.source_repo.put()
    imp = importer.Importer('fake_public_key', 'fake_private_key', self.tmp_dir,
                            importer.DEFAULT_PUBLIC_LOGGING_BUCKET, 'bucket',
  ...
  def test_no_updates(self, unused_mock_time: mock.MagicMock,
    ...
    self.source_repo.last_update_date = datetime.datetime(2024, 2, 1, tzinfo=datetime.UTC)
    repo = self.source_repo.put()
    imp = importer.Importer('fake_public_key', 'fake_private_key', self.tmp_dir,
                            importer.DEFAULT_PUBLIC_LOGGING_BUCKET, 'bucket',
                            True, False)
